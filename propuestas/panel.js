// panel.js v9.1 — consola estilo chat, revisada por el doc
const AresPanel = {
  init: () => {
    try {
        ...
        if (typeof original === 'function') {
          AresCerebro.mostrar = (q, m) => {
            try {
              ...
              if (typeof AresDiag !== 'undefined') AresDiag.log('AresPanel error en mostrar: ' + e.message);
            }
          };
        }
      }
      ...
      if (ta3) {
        ...
        ta3.addEventListener('input', () => {
          try {
            ...
            ta3.style.height = Math.min(ta3.scrollHeight, 96) + 'px';
          } catch (e) {
            if (typeof AresDiag !== 'undefined') AresDiag.log('AresPanel error en input: ' + e.message);
          }
        });
        ...
        ta3.addEventListener('keydown', (e) => {
          try {
            if (e.key === 'Enter' && !e.shiftKey) {
              ...
              if (btnEnviar) {
                btnEnviar.click();
              } else if (typeof AresCerebro !== 'undefined' && typeof AresCerebro.enviar === 'function') {
                AresCerebro.enviar();
              }
            }
          } catch (err) {
            if (typeof AresDiag !== 'undefined') AresDiag.log('AresPanel error en keydown: ' + err.message);
          }
        });
      }

      if (typeof AresDiag !== 'undefined' && typeof AresDiag.log === 'function') {
        AresDiag.log('AresPanel v9.1 inicializado correctamente.');
      }
    } catch (err) {
      console.error('Error crítico al inicializar AresPanel:', err);
    }
  }
};
